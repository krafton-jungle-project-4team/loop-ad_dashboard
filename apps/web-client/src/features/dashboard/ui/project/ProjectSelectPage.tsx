...
export function ProjectSelectPage() {
  ...
  return (
    <main className="min-h-svh bg-[#21192b] text-white">
      <div className="mx-auto flex min-h-svh w-full max-w-7xl flex-col px-5 py-5 sm:px-8">
        ...
        <section className="flex flex-1 flex-col justify-center gap-8 py-12">
          <div className="grid max-w-3xl gap-3">
            <span className="text-xs font-semibold uppercase tracking-[0.18em] text-[#f0a36c]">
              Projects / Overview
            </span>
          ...
          <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <ProjectManagementDialog
              trigger={
                <Button
                  aria-label="프로젝트 관리"
                  className="group min-h-40 rounded-lg border-dashed border-white/20 bg-white/5 p-4 text-white shadow-none hover:border-[#a596ff] hover:bg-white/10 hover:text-white"
                  type="button"
                  variant="outline"
                >
...
function ProjectCard({ index, project }: { index: number; project: DashboardProject }) {
  ...
  return (
    <Link
      className="group block rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a596ff]"
      params={{ projectId: project.project_id }}
      to="/dashboard/$projectId"
    >
      <Card className="min-h-40 border-white/10 bg-[#30253c] p-0 text-white shadow-none transition-[border-color,transform,box-shadow] group-hover:-translate-y-0.5 group-hover:border-[#a596ff]/60 group-hover:shadow-[0_10px_30px_rgb(0_0_0_/_0.24)]">
        <CardContent className="grid h-full grid-cols-[auto_minmax(0,1fr)] items-center gap-4 p-4 text-left">
          ...
          <div className="grid min-w-0 gap-1.5">
            ...
            <Badge
              className="mt-1 bg-[#463754] text-[#d8d0ff] hover:bg-[#463754]"
              variant="secondary"
            >
...
function ProjectCardSkeletons() {
  return Array.from({ length: 4 }, (_, index) => (
    <Card className="min-h-40 border-white/10 bg-[#30253c] p-0" key={index}>
