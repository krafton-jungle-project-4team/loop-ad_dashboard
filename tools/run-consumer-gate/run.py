...
from report import digest, validate_lane, verdict, write_inventory, verify_inventory
...
def run(args):
    ...
    with tempfile.TemporaryDirectory(prefix='rcc-preparation-') as temporary:
        work = Path(temporary)
        try:
            ...
            producer_output = args.producer_output.resolve() if args.producer_output else work / 'producer'
            ...
            # Preserve partial producer evidence on failure outside the CI upload allowlist.
            if (work / 'producer').is_dir():
                shutil.copytree(work / 'producer', diagnostics / 'producer', dirs_exist_ok=True)
